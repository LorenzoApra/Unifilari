...
function availablePanelSockets(panel, target, ignoredLink = null) {
  const required = requiredPlug(target), used = new Set(state.links.filter((link) => link.id !== ignoredLink && link.from === panel.id).map((link) => nodeById(link.to)?.socket).filter(Boolean));
